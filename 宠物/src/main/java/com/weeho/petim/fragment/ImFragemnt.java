...
public class ImFragemnt extends BaseFragment {
    ...
    //本地存储未读条数
    private ArrayList<NotReadList.NotReadBean> list_num = new ArrayList<NotReadList.NotReadBean>();
...
//    private String mReceive_hxUserName;
    ...
    private void GetNotReadNum() {
        if(!StringUtil.isEmpty(hxUserName)) {
            EMConversation conversation = EMClient.getInstance().chatManager().getConversation(hxUserName);
            int msgCountnum = conversation.getUnreadMsgCount();
            MainActivity mainActivity = (MainActivity) getActivity();
            myHandler =  mainActivity.handler;
            Message message = new Message();
            message.what = ConstantsUtil.CONTENT_SUCCESS_THREE;
            message.arg1 = msgCountnum;
...
//                    conversationListView.refresh();
                ...
                case ConstantsUtil.CONTENT_SUCCESS:
                    if(hxImBaseBean != null && hxImBaseBean.getResult()!= null) {
                        UpdateNum();
                        String hxTo = hxImBaseBean.getResult().getHxusername();
...
private String input_num = "0";
    private void UpdateNum(){
        if(!StringUtil.isEmpty(typeUpdate)){
            ...
            mTotalNum = mTotalNum-Integer.parseInt(input_num);
...
//            SetTotalNumView();
            //保存总条数
            SharedPreferencesUtil.writeSharedPreferencesInt(getActivity(),"totalNum","totalNum",mTotalNum);
...
//        if(mTotalNum>0) {
//            MainActivity.unreadAddressLable.setText(mTotalNum+"");
//            MainActivity.unreadAddressLable.setVisibility(View.VISIBLE);
//        }else{
//            MainActivity.unreadAddressLable.setVisibility(View.INVISIBLE);
//        }
//    }
...
//        ArrayList<Map<String,String>> list = (ArrayList<Map<String, String>>) SharedPreferencesUtil.get(getActivity(),"msgHistory","msgHistory");
    }
...
//                           mReceive_hxUserName = conversation.conversationId();
//                           if(currt_time == 0) {
//                               currt_time = System.currentTimeMillis();
//                           }else{
//                               //如果再次接受到消息时间间隔大于3分钟 自动断开 否则 更新本地时间
//                              if((System.currentTimeMillis()-currt_time) > 1000 * 60 * 3){
//                                  mReceive_hxUserName = null;
//                              } else{
//                                  currt_time = System.currentTimeMillis();
//                              }
//                           }
...
//        mTotalNum = SharedPreferencesUtil.getSharedPreferencesInt(getActivity(),"totalNum","totalNum",0);
        mTotalNum = 0;
