...
public class AddOutdoorAdapter extends RecyclerView.Adapter<AddOutdoorAdapter.ViewHolder>{
    //定义成就相关信息
    ArrayList<BadHabitDicBean.ResultBean.DataBean.OutdoorBean> listindoor;
    private View view;
    private AddOutdoorAdapter.ViewHolder viewHolder;
    private Activity mActivity;
    public AddOutdoorAdapter(ArrayList<BadHabitDicBean.ResultBean.DataBean.OutdoorBean> listindoor, Activity mActivity) {
        this.listindoor = listindoor;
        this.mActivity = mActivity;
    }

    @Override
    public AddOutdoorAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        view = LayoutInflater.from(parent.getContext()).inflate(R.layout.achieve_item,parent,false);
        viewHolder = new AddOutdoorAdapter.ViewHolder(view);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(final AddOutdoorAdapter.ViewHolder holder, final int position) {
        BadHabitDicBean.ResultBean.DataBean.OutdoorBean coorec = listindoor.get(position);
